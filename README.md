...
Each workspace has its own switch, and both default to `Disabled`:

| Switch | Deploys |
| --- | --- |
| `deployOperationalLogAnalyticsWorkspace` | `das-{env}-operational-la` |
| `deploySecurityLogAnalyticsWorkspace` | `das-{env}-security-la`, and gates the archive, the Splunk export and the Event Hubs namespace, which all read from it |

The pipeline runs `Deploy_PROD` and `Deploy_MO` off `Build` rather than off `Deploy_PREPROD`, so every environment deploys in parallel from the same master build and there is no stage ordering that stops a rollout partway. The switches are what make an environment at a time rollout possible: merge with them off everywhere, then enable one variable group at a time.

DTA is out of scope. It is excluded by leaving the switches off on its variable group, so the seven environments in the LLD are the ones that get workspaces.
...
Long term retention is applied per table via `securityLogAnalyticsTableNames`, which defaults to `AZKVAuditLogs,AzureActivity`.

**Azure tables** are pre registered by schema in a Log Analytics workspace rather than created on first ingestion - a workspace created in April 2025 that has only ever received firewall logs still carries 685 tables, including ones it has never had data in. Long term retention can therefore be applied from deployment, before any resource is onboarded, and retention on an empty table costs nothing because it is billed per GB retained.

| Table | Type | Populated when |
| --- | --- | --- |
| `AZKVAuditLogs` | Azure | A diagnostic setting sets `logAnalyticsDestinationType: Dedicated` |
| `AzureActivity` | Azure | Subscription activity log is exported to the workspace |
| `AzureDiagnostics` | **Custom** | A diagnostic setting uses the default destination type |

**`AzureDiagnostics` cannot go in this list.** It is a [custom table created by the ingestion pipeline on first write](https://learn.microsoft.com/azure/azure-monitor/reference/tables/azurediagnostics), it cannot be created by an ARM template or the tables API, and its retention cannot be set before it exists. Listing it fails the deployment with a Bad Request against an empty workspace. Onboarding must therefore either set `logAnalyticsDestinationType: Dedicated`, which is preferred anyway, or set `AzureDiagnostics` retention as a follow up step after the first log arrives.

That preference is not only a deployment convenience. `AzureDiagnostics` is a shared table that many resource types write into, so anything retained or exported from it applies to all of them. `AZKVAuditLogs` is Key Vault only and can be targeted precisely. Data export cannot filter rows, so the resource specific route is the only way to send one resource type to Splunk without the others.
...
| State | Behaviour |
| --- | --- |
| `Disabled` | No immutability. The account can be deleted freely. |
